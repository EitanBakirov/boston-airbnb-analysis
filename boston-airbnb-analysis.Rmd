...
response_counts <- as.data.frame(table(listings_data$property_type))
names(response_counts) <- c("property_type", "count")

# Calculate percentages
total_listings <- sum(response_counts$count)
response_counts$percentage <- (response_counts$count / total_listings) * 100

# Create the bar plot
ggplot(response_counts, aes(x = reorder(property_type, -count), y = count)) +
  geom_bar(stat = "identity", fill = "orange", color = "black") +
  geom_text(aes(label = paste0(count, "\n(", round(percentage, 1), "%)")),
            vjust = -0.5, size = 5.5, fontface = "bold") +
  scale_y_continuous(
    breaks = seq(0, max(response_counts$count), by = 250),
    limits = c(0, max(response_counts$count) + 250)
  ) +
  labs(title = "Distribution of Property Types", x = "Property Type", y = "Frequency") +
  theme_minimal() +
  theme(
    ...
    panel.grid.major.y = element_line(color = "black", size = 0.1) # Add horizontal grid lines for y-axis
  )
```
...
ggplot(correlations, aes(x = reorder(Aspect, Correlation), y = Correlation, fill = Aspect)) +
  ...
  geom_text(aes(label = round(Correlation, 2)), 
            position = position_stack(vjust = 1.07), size = 4, fontface = "bold", color = "black") +
  coord_flip() +
  labs(title = "Correlation of Various Review Aspects with Overall Review Score Rating", 
       x = "Review Aspect", 
       y = "Correlation Coefficient") +
  scale_y_continuous(
    breaks = seq(0, 1, by = 0.2),  # Add more numbers on y-axis
    limits = c(0, 1),
    expand = expansion(mult = c(0, 0.05))  # Add some space at the top
  ) +
  theme_minimal() +
  theme(
    ...
    plot.title = element_text(hjust = 1.3, face = "bold", size = 12),
    legend.position = "none",
    panel.grid.major.y = element_line(color = "gray", size = 0.5),  # Add grid lines
...
# Filter listings_data for property_type 'Apartment'
apartment_data <- listings_data %>%
  filter(property_type == "Apartment")

# Ensure that beds are treated as factors to include NA if necessary
apartment_data$beds <- as.character(apartment_data$beds)
apartment_data$beds[is.na(apartment_data$beds)] <- "NA"
apartment_data$beds <- factor(apartment_data$beds, levels = c("NA", as.character(0:16)))

# Calculate average price_dollars for each neighborhood and bed number
average_price_per_neighborhood <- apartment_data %>%
  group_by(neighbourhood, beds) %>%
  summarise(average_price = mean(price_dollars, na.rm = TRUE), .groups = 'drop')

```

```{r}
# List of unique bed numbers
bed_numbers <- unique(average_price_per_neighborhood$beds)

# Create a plot for each bed number
plots <- lapply(bed_numbers, function(bed) {
  # Filter data for the specific bed number
  bed_data <- average_price_per_neighborhood %>%
    filter(beds == bed)
  
  # Create the bar plot
  p <- ggplot(bed_data, aes(x = reorder(neighborhood, -average_price), y = average_price)) +
    geom_bar(stat = "identity", fill = "#EF562D") +
    geom_text(aes(label = round(average_price, 2)), vjust = -0.5, size = 3.5) +
    labs(title = paste("Average Price for Apartments with", bed, "Beds"),
         x = "Neighborhood",
...
```{r}
# Ensure that beds are treated as factors to include NA if necessary
average_price_per_neighborhood$beds <- as.character(average_price_per_neighborhood$beds)
average_price_per_neighborhood$beds[is.na(average_price_per_neighborhood$beds)] <- "NA"
average_price_per_neighborhood$beds <- factor(average_price_per_neighborhood$beds, levels = c("NA", as.character(0:16)))

# Create the faceted bar plot
ggplot(average_price_per_neighborhood, aes(x = reorder(neighbourhood, -average_price), y = average_price)) +
  geom_bar(stat = "identity", fill = "#EF562D") +
  geom_text(aes(label = round(average_price, 2)), vjust = -0.5, size = 3.5) +
  facet_wrap(~ beds, scales = "free_y", ncol = 4) +  # Create a facet for each bed number
  labs(title = "Average Price for Apartments by Number of Beds and Neighborhood",
       x = "Neighborhood",
       y = "Average Price (in dollars)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
```{r}
# Create a new data frame from listings_data
new_data <- listings_data

# Convert beds to character and then to factor to handle NA properly
new_data$beds <- as.character(new_data$beds)
new_data$beds[is.na(new_data$beds)] <- "NA"
new_data$beds <- factor(new_data$beds, levels = c("NA", as.character(0:16)))

# Calculate average price_dollars and count of listings for each number of beds
average_price_per_bed <- new_data %>%
  group_by(beds) %>%
  summarise(
    average_price = mean(price_dollars, na.rm = TRUE),
    count = n(),  # Count of listings for each bed number
    .groups = 'drop'
  )

# Create a dummy data frame for legend
legend_data <- data.frame(
  beds = factor(c("NA", "0"), levels = levels(average_price_per_bed$beds)),
  average_price = c(NA, NA),
  count = c(NA, NA),
  text_type = c("Average Price", "Count")
)

# Combine actual and dummy data for plotting
plot_data <- bind_rows(
  average_price_per_bed %>% mutate(text_type = "Average Price"),
  average_price_per_bed %>% mutate(text_type = "Count"),
  legend_data
)


# Create the bar plot
ggplot(average_price_per_bed, aes(x = beds, y = average_price)) +
  geom_bar(stat = "identity", fill = "#EF562D") +
  geom_text(aes(label = round(average_price, 2), color = "Average Price"), 
            size = 2.5, vjust = -0.5, fontface = "bold", show.legend = TRUE) +
  geom_text(aes(label = count, color = "Count"), 
            size = 2.5, vjust = 1.5, show.legend = TRUE) +
  scale_color_manual(name = "Legend", values = c("Average Price" = "blue", "Count" = "green")) +
  labs(title = "Average Price by Number of Beds",
       x = "Number of Beds",
       y = "Average Price (in dollars)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
...
```{r}
# Create a new data frame from listings_data
property_data <- listings_data

# Calculate average price_dollars and count of listings for each property type
average_price_per_property <- property_data %>%
  group_by(property_type) %>%
  summarise(
    average_price = mean(price_dollars, na.rm = TRUE),
    count = n(),  # Count of listings for each property type
    .groups = 'drop'
  )

# Create a dummy data frame for legend
legend_data_property <- data.frame(
  property_type = factor(c("Dummy1", "Dummy2"), levels = levels(average_price_per_property$property_type)),
  average_price = c(NA, NA),
  count = c(NA, NA),
  text_type = c("Average Price", "Count")
)
...
# Combine actual and dummy data for plotting
plot_data_property <- bind_rows(
  average_price_per_property %>% mutate(text_type = "Average Price"),
  average_price_per_property %>% mutate(text_type = "Count"),
  legend_data_property
)

# Create the bar plot
ggplot(average_price_per_property, aes(x = reorder(property_type, -average_price), y = average_price)) +
  geom_bar(stat = "identity", fill = "#EF562D") +
  geom_text(aes(label = round(average_price, 2), color = "Average Price"), 
            size = 2.5, vjust = -0.5, fontface = "bold", show.legend = TRUE) +
  geom_text(aes(label = count, color = "Count"), 
...
# Calculate median price_dollars and count of listings for each property type
median_price_per_property <- property_data %>%
  group_by(property_type) %>%
  summarise(
...
# Create a dummy data frame for legend
legend_data_property <- data.frame(
  property_type = factor(c("Dummy1", "Dummy2"), levels = levels(median_price_per_property$property_type)),
  median_price = c(NA, NA),
  count = c(NA, NA),
  text_type = c("Median Price", "Count")
)

# Combine actual and dummy data for plotting
plot_data_property <- bind_rows(
  median_price_per_property %>% mutate(text_type = "Median Price"),
  median_price_per_property %>% mutate(text_type = "Count"),
  legend_data_property
)

# Create the bar plot
ggplot(median_price_per_property, aes(x = reorder(property_type, -median_price), y = median_price)) +
  geom_bar(stat = "identity", fill = "#EF562D") +
  geom_text(aes(label = round(median_price, 2), color = "Median Price"), 
            size = 2.5, vjust = -0.5, fontface = "bold", show.legend = TRUE) +
  geom_text(aes(label = count, color = "Count"), 
            size = 2.5, vjust = 1.5, show.legend = TRUE) +
  scale_color_manual(name = "Legend", values = c("Median Price" = "blue", "Count" = "green")) +
...
# Calculate average price_dollars for each number of beds
average_price_per_bed <- apartment_data %>%
  group_by(beds) %>%
  summarise(average_price = mean(price_dollars, na.rm = TRUE), .groups = 'drop')
...
ggplot(average_price_per_bed, aes(x = beds, y = average_price)) +
  geom_bar(stat = "identity", fill = "#EF562D") +
  geom_text(aes(label = round(average_price, 2)), vjust = -0.5, size = 3.5) +
  labs(title = "Average Price for Apartments by Number of Beds",
       x = "Number of Beds",
       y = "Average Price (in dollars)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
ggplot(neighborhood_revpan_data, aes(x = year_month, y = occupancy_rate * 100, color = neighbourhood, group = neighbourhood)) +
  geom_line(size = 1.2) +
  geom_point(size = 3) +
  geom_text(data = label_data, aes(label = neighbourhood), 
            hjust = 0, vjust = -0.5, size = 5, check_overlap = TRUE, color = "black") +
  ...
  theme(
    ...
    legend.position = c(1.2, 0.5),  # Adjusting legend position
    legend.justification = c(0.9, 0.5),
    plot.margin = unit(c(1, 15, 1, 1), "lines") # Adjusting the plot margins
  ) +
...
```{r}
# Convert year_month to a date type
revpan_data <- revpan_data %>%
  mutate(year_month = ymd(paste0(year_month, "-01")))
```

```{r}

# Convert year_month to a date type for plotting (but keep original for labeling)
revpan_data <- revpan_data %>%
...
scaling_factor <- max(revpan_summary$avg_revpan, na.rm = TRUE) / max(monthly_avg_occupancy$avg_occupancy_rate_percentage, na.rm = TRUE)

# Create the combined plot with dual y-axes
ggplot() +
  # Plot RevPAN
  geom_line(data = revpan_summary, aes(x = year_month_date, y = avg_revpan), color = "blue", size = 1) +
  geom_point(data = revpan_summary, aes(x = year_month_date, y = avg_revpan), color = "blue", size = 2) +
  # Plot Occupancy Rate with scaling
  geom_line(data = monthly_avg_occupancy, aes(x = year_month_date, y = avg_occupancy_rate_percentage * scaling_factor), color = "red", size = 1) +
  geom_point(data = monthly_avg_occupancy, aes(x = year_month_date, y = avg_occupancy_rate_percentage * scaling_factor), color = "red", size = 2) +
  # Add secondary y-axis
  scale_y_continuous(
    name = "Average RevPAN (Revenue per Available Night)",
    sec.axis = sec_axis(~ . / scaling_factor, name = "Occupancy Rate (%)")
  ) +
  scale_x_date(labels = revpan_summary$year_month, breaks = revpan_summary$year_month_date) +
  labs(title = "Monthly RevPAN and Occupancy Rate",
       x = "Month") +
  # Add annotations for the lines
  annotate("text", x = min(revpan_summary$year_month_date), y = max(revpan_summary$avg_revpan), label = "Average RevPAN", color = "blue", angle = 71, vjust = 5.3, hjust = 1.7) +
  annotate("text", x = min(monthly_avg_occupancy$year_month_date), y = max(monthly_avg_occupancy$avg_occupancy_rate_percentage * scaling_factor), label = "Occupancy Rate", color = "red", angle = 73, vjust = 9.5, hjust = 2.6) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
...
```
```{r}
# Calculate occupancy rate per day and additional metrics
daily_occupancy_2 <- merged_data %>%
  group_by(date) %>%
  summarise(
    total_listings = n(),  # Total listings on this day
    bookings = sum(available_category == 1, na.rm = TRUE),  # Listings booked on this day
    occupancy_rate = (bookings / total_listings) * 100,  # Occupancy percentage
    total_price = sum(price_dollars, na.rm = TRUE),  # Sum of price_dollars for each day
    median_price = median(price_dollars, na.rm = TRUE),  # Median price for each day
    average_price = mean(price_dollars, na.rm = TRUE)  # Average price for each day
  )
...
ggplot(daily_occupancy, aes(x = date, y = occupancy_rate)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  ylim(0, 100) +
  annotate("rect", xmin = as.Date("2017-04-12"), xmax = as.Date("2017-04-20"),
           ymin = 0, ymax = 100, alpha = 0.2, fill = "red") +
  annotate("text", x = as.Date("2017-04-15"), y = 80, label = "Abnormal Drop", color = "red", size = 4)
```
```{r}
...
scaling_factor <- 100 / max(average_price_per_day$average_price, na.rm = TRUE)

# Create a combined plot with dual y-axes and colored y-axis labels
ggplot() +
  # First plot: Daily Occupancy Rate
  geom_line(data = daily_occupancy, aes(x = date, y = occupancy_rate), color = "blue", size = 1) +
  geom_point(data = daily_occupancy, aes(x = date, y = occupancy_rate), color = "blue", size = 1) +
  geom_rect(aes(xmin = as.Date("2017-04-05"), xmax = as.Date("2017-04-25"),
                ymin = 0, ymax = 100), alpha = 0.2, fill = "red") +
  scale_x_date(date_labels = "%b %Y", date_breaks = "1 month") +
  geom_text(aes(x = as.Date("2017-04-15"), y = 30), label = "Abnormal Behavior", color = "red", size = 4) +
  scale_y_continuous(
    name = "Occupancy Rate (%)",
    limits = c(0, 100),
    sec.axis = sec_axis(~ . / scaling_factor, name = "Average Price ($)")
  ) +
  # Second plot: Daily Average Price
  geom_line(data = average_price_per_day, aes(x = date, y = average_price * scaling_factor), color = "orange", size = 1) +
  geom_point(data = average_price_per_day, aes(x = date, y = average_price * scaling_factor), color = "orange", size = 1) +
  # Common elements
  labs(title = "Daily Occupancy Rate and Average Price",
       x = "Date") +
  theme_minimal() +
  theme(
  ...
  # ) +
  # annotate("text", x = as.Date("2016-10-01"), y = 15, 
  #          label = "Occupancy~Rate == frac('# bookings per day', '# listings per day')", 
  #          parse = TRUE, color = "blue", size = 3, hjust = 0)
...
ggplot(price_counts, aes(x = price_bin, y = count)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = count), vjust = -0.5, size = 5) +  # Enlarged text for counts
  labs(title = "Distribution of Listing Prices",
       x = "Price (in dollars)",
       y = "Count of Listings") +
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, size = 12),  # Enlarged x-axis text
    axis.text.y = element_text(size = 12),  # Enlarged y-axis text
    axis.title.x = element_text(size = 14),  # Enlarged x-axis title
    axis.title.y = element_text(size = 14),  # Enlarged y-axis title
    plot.title = element_text(size = 16, face = "bold")  # Enlarged plot title
  )
```
...
# Scatter plot to visualize all listing prices by neighborhood
ggplot(listings, aes(x = reorder(neighbourhood, price_dollars, median), y = price_dollars)) +
  geom_point(aes(color = price_dollars >= 1000), alpha = 0.6) +
  scale_color_manual(values = c("FALSE" = "blue", "TRUE" = "red"), labels = c("< 1000", ">= 1000")) +
  geom_hline(yintercept = 1000, linetype = "dotted", color = "red", size = 1) +
  labs(title = "Listing Prices by Neighborhood",
       x = "Neighborhood",
       y = "Price (in dollars)",
       color = "Price") +
...
```{r}
# Step 1: Filter listings_data to get listing_ids where price_dollars is above 999
high_price_listings <- listings_data %>%
  filter(price_dollars > 999) %>%
  select(id)

# Convert to vector to avoid any potential type issues
high_price_listings_vector <- high_price_listings$id

# Step 2: Ensure available_category is treated as a character in calendar_data
calendar_data <- calendar_data %>%
  mutate(available_category = as.character(available_category))

# Step 3: Use these listing_ids to filter calendar_data
filtered_calendar_data <- calendar_data %>%
  filter(listing_id %in% high_price_listings_vector, available_category == "1")

# Step 4: Count the number of rows in calendar_data where available_category is "1" for each listing_id
available_counts <- filtered_calendar_data %>%
  group_by(listing_id) %>%
  summarise(count_available_days = n())
...
```{r}
# Step 1: Filter listings_data to get listing_ids where price_dollars is above 999
o_beds_listings <- listings_data %>%
  filter(beds == 0) %>%
  select(id)

# Convert to vector to avoid any potential type issues
o_beds_listings_vector <- o_beds_listings$id
...
# Step 3: Use these listing_ids to filter calendar_data
filtered_calendar_data <- calendar_data %>%
  filter(listing_id %in% o_beds_listings_vector, available_category == "1")
...
```{r}
# Step 1: Filter listings_data to get listing_ids where price_dollars is above 999
no_beds_listings <- listings_data %>%
  filter(is.na(beds)) %>%
  select(id)

# Convert to vector to avoid any potential type issues
no_beds_listings_vector <- no_beds_listings$id
...
# Step 3: Use these listing_ids to filter calendar_data
filtered_calendar_data <- calendar_data %>%
  filter(listing_id %in% no_beds_listings_vector, available_category == "1")
...
```
```{r}
# Calculate the number of listings with one bed
num_one_bed_listings <- listings_data %>% filter(beds == 1) %>% nrow()

# Calculate the total number of beds
total_beds <- sum(listings_data$beds)

# Calculate the proportion
proportion_one_bed <- num_one_bed_listings / total_beds

proportion_one_bed
```
```{r}
# Convert date to Date type if not already
calendar_data$date <- as.Date(calendar_data$date, format = "%m/%d/%Y")

# Calculate the number of available and booked days per listing
availability_summary <- calendar_data %>%
  group_by(listing_id) %>%
  summarise(
    total_days = n(),
    available_days = sum(available_category == 1, na.rm = TRUE),
    booked_days = sum(available_category == 0, na.rm = TRUE)
  )

# Calculate the average available and booked days across all listings
average_dates <- availability_summary %>%
  summarise(
    avg_available_days = mean(available_days, na.rm = TRUE),
    avg_booked_days = mean(booked_days, na.rm = TRUE)
  )
